...
public partial class andhra : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Getuserpred(shortcode: "666999");
        }
        else if (Request.QueryString["scode"] != null)
        {
            txtprdcsts.InnerText = "Prediction By You";
            Getuserpred(shortcode: Request.QueryString["scode"]); 
        }


    }

    public void Getuserpred(string shortcode)
    {
        aPCONSISTENCIEs = dAL.Userslist();
        foreach (var item in aPCONSISTENCIEs.Where(y => y.Shot_code == shortcode))
        {
            Predicts = item.Userpredicts;
            updcts = Predicts.Split(',');
            for (int i = 0; i < updcts.Length; i++)
                if (updcts[i] == "" || updcts[i] == null)
                    updcts[i] = "0";
            lbltdp.Text = updcts[0];
            lblysr.Text = updcts[1];
            lbljanasena.Text = updcts[2];
    ...
    protected void btn_upredict_Click(object sender, EventArgs e)
    {
        ...
        if (Session["email"] != null)
        {
            ...
            U_p[0] = Convert.ToInt16(txttdp.Text);
            U_p[1] = Convert.ToInt16(txtysr.Text);
            U_p[2] = Convert.ToInt16(txtjana.Text);
